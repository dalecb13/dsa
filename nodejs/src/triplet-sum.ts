import { twoPointerTechnique } from "./two-pointer-technique";

const tripletSumNaive = (array: number[], targetSum: number): [number, number, number][] => {
  const result: Set<[number, number, number]> = new Set<[number, number, number]>();
...
const optimizedTripletSum = (array: number[], targetSum: number) => {
  array.sort();
  const result: Set<[number, number, number]> = new Set<[number, number, number]>();
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    const twoPointer = twoPointerTechnique(subArray, targetSum - triplet[0]);
